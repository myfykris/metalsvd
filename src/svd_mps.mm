...
void load_kernels(id<MTLLibrary> lib, SVDKernels& k, NSString* suffix, bool required) {
    ...
    if (required) {
        TORCH_CHECK(k.jacobi && k.norm && k.normalize, "Failed to load required kernels for suffix: ", [suffix UTF8String]);
    }
}
...
std::vector<torch::Tensor> svd_forward(torch::Tensor A) { 
    ...
    torch::Tensor PairsTens = torch::tensor(pairs_cpu, torch::dtype(torch::kInt32).device(torch::kCPU));
    ...
    // Shared mem size depends on sizeof(T). 
    // Float/Half/BFloat. 
    // Wait, shared mem in kernel is "threadgroup T*". 
    // sizeof(T) varies.
    int elem_size = A.element_size(); // 4 or 2
    NSUInteger sharedMemSize = ((threads_per_group_val + 31) / 32) * 3 * elem_size; 
    // + 2 elements for c,s? No, strictly reusing part_ii slots in my kernel logic.
    // Check kernel: shared_mem[0] = c; shared_mem[1] = s; 
    // shared_mem is T*.
    // Ensure size is sufficient.
    
    uint32_t M_u = (uint32_t)M;
    uint32_t N_u = (uint32_t)N;
    // Strides in ELEMENTS, not bytes, for pointer arithmetic?
    // Metal pointer arithmetic is typed. So strides should be in ELEMENTS.
    ...
    // 5. Compute Norms
    torch::Tensor S = torch::empty({Batch, N}, A.options()); // Same dtype as A
    // S is T*.
